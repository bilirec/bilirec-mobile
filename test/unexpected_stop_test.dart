...
void main() {
  ...
  test('重啟且未開開機自啟時引導開啟自啟，已開自啟或後台被殺時導向設定教學', () {
    expect(
  ...
  test('未開自啟的重啟提示不提及後台被殺或設定教學', () {
    for (final key in [
      'unexpectedStopRebootTitle',
      'unexpectedStopRebootBody',
    ]) {
      for (final label in labelsForKey(key)) {
        expect(label.contains('設定教學'), isFalse);
        expect(label.contains('设置教程'), isFalse);
        expect(label.contains('背景被殺'), isFalse);
        expect(label.contains('后台被杀'), isFalse);
      }
    }
  });
}
