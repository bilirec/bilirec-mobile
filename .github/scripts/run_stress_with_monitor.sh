...
adb uninstall org.bilirec.bilirec >/dev/null 2>&1 || true
...
echo "=== 預授權 legacy storage（Android 10 / API 29）==="
adb shell pm grant org.bilirec.bilirec android.permission.READ_EXTERNAL_STORAGE 2>/dev/null || true
adb shell pm grant org.bilirec.bilirec android.permission.WRITE_EXTERNAL_STORAGE 2>/dev/null || true
...
flutter test "$TEST_FILE" \
  --reporter=expanded \
  --dart-define=CI="$CI_FLAG" \
  --no-uninstall \
  -d emulator-5554 &
FLUTTER_PID=$!

sh ./.github/scripts/watch_app_process.sh org.bilirec.bilirec "$FLUTTER_PID" 90 10 240 &
WATCHDOG_PID=$!
